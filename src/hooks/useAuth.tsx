...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (token) {
      // 尝试从 localStorage 恢复用户信息，后续可加 /api/auth/me 接口验证
  ...
  const login = useCallback(async (req: LoginReq) => {
    const data = await apiClient.request<LoginResp>("/api/auth/login", {
      method: "POST",
      body: req,
    });
  ...
  const register = useCallback(async (req: RegisterReq) => {
    const data = await apiClient.request<UserBrief>("/api/auth/register", {
      method: "POST",
      body: req,
    });
    return data;
