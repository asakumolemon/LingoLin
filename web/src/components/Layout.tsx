    ...
    <div className="flex h-screen flex-col">
      <header className="flex h-14 items-center justify-between border-b border-gray-100 bg-white/80 px-6 backdrop-blur">
        <div className="flex items-center gap-3">
          ...
          <span className="rounded-md bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-600">管理端</span>
        </div>
        <div className="flex items-center gap-3">
          {user && (
            <>
              <span className="text-sm text-gray-400">{user.username}</span>
              <button onClick={logout} className="rounded-lg px-3 py-1.5 text-sm text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-700">退出</button>
            </>
          )}
        </div>
      </header>
